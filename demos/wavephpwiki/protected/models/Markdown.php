...
class Markdown {
    ...
    //按分类获得博客列表
    public function getBlogListByCategory($categoryId) {
        $cacheKey = "getBlogByCategory_" . $categoryId . ".gb";
        $blogList = $this->gbReadCache($cacheKey);
        ...
        if ($blogObj === false) {
            foreach ($this->blogs as $idx => $blog) {
                if ($blog['blogId'] == $blogId) {
                    $serverPath = $blog['serverPath'];
                    //读取自定义博客属性信息
                    $blogProp = $this->readPostBaseInfo($serverPath, true);
                    $blogObj = array_merge($blog, $blogProp);
    ...
    public function initAllBlogData($postPath, $enableCache=false) {
        ...
        $this->cacheDir = ROOT_PATH.'/data/caches/';
    ...
    //读取博客的基本信息
    private function readPostBaseInfo($serverPath, $isGetContent = false) {
        $keywrodsArr = array();
        $tagsArr = array();
        $cateArr = array();
    ...
    private function readAllPostInfo($mdfiles, $postPath) {
        foreach ($mdfiles as $idx => $fileProp) {
            ...
            $siteURL = $this->changeFileExt($relativePath);
